...
export class BeaconModule implements AtlasModule {
  ...
  onLoad(context: ModuleContext): void {
    ...
    group.name = 'BeaconModel';
    group.position.set(0, 0.75, -0.7);
  ...
  private async loadModel(): Promise<void> {
    ...
    try {
      ...
      this.fitModelToSize(this.model, 3.2);
      this.group.add(this.model);
      this.updateCollider();
    } catch (error) {
      console.warn('[beacon] Failed to load model', error);
    }
  }
  ...
  private updateCollider(): void {
    ...
    if (!this.collider) {
      ...
      this.collider.name = 'BeaconCollider';
